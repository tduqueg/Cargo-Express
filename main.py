...
async def obtener_metricas(token: str = Depends(oauth2_scheme)):
    ...
    try:
        ...
        cursor.execute('''
            SELECT id_repartidor, TO_CHAR(fecha_entrega, 'YYYY-MM-DD') as dia, COUNT(*) as entregas
            FROM pedidos
        ...
        cursor.execute('''
        SELECT TO_CHAR(fecha_entrega, 'YYYY-MM-DD') as dia, COUNT(*) as total_entregas
        FROM pedidos
        GROUP BY dia
...
async def monitoreo(request: Request):
    ...
    # Cambia 'strftime' por 'to_char' para PostgreSQL
    cursor.execute('''
    SELECT id_repartidor, to_char(fecha_entrega, 'HH24') as hora, COUNT(*) as entregas
    FROM pedidos
